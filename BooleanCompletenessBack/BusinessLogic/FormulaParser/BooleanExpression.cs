    ...
    public class BooleanExpression
    {
        ...
        public bool Evaluate(List<string> paramNames, List<bool> paramValues)
        {
            if (paramNames.Count != paramValues.Count)
                throw new ArgumentException("Параметры и значенияния не совпадают");
            ...
            foreach (var token in _rpn)
            {
                ...
                else if (token.Type == TokenType.Operator)
                {
                    ...
                    {
                        if (stack.Count < 1) throw new Exception("Неверное RPN");
                        bool a = stack.Pop();
                        stack.Push(EvalUnary(token.Value, a));
                    }
                    else
                    {
                        if (stack.Count < 2) throw new Exception("Неверное RPN");
                        bool b = stack.Pop();
                        bool a = stack.Pop();
            ...
            if (stack.Count != 1) throw new Exception("Неверное преобразование");
            return stack.Pop();
        }
        ...
        {
            return op == "¬" ? !a : throw new Exception("Нераспознанная унарная операция");
        }
        ...
        private bool EvalBinary(string op, bool a, bool b)
        {
            switch (op)
            {
                ...
                case "↛": return a && !b; 
                case "↮": return a != b; 
                default: throw new Exception($"Нераспозанный оператор {op}");
            }
        }
        ...
        private bool NodesEqual(ExpressionNode node1, ExpressionNode node2)
        {
            ...
            {
                if (bin1.Operator != bin2.Operator) return false;
                ...
                if (op == "∧" || op == "∨")
                {
                    ...
                    if (ops1.Count != ops2.Count) return false;
                    for (int i = 0; i < ops1.Count; i++)
                    {
                        if (!NodesEqual(ops1[i], ops2[i])) return false;
                    }
                    return true;
                }
                else
        ...
        private bool IsAbsorbed(ExpressionNode term1, ExpressionNode term2)
        {
           
            if (term2 is BinaryNode bin2 && bin2.Operator == "∧")
            {
                if (NodesEqual(term1, bin2.Left) || NodesEqual(term1, bin2.Right))
                    return true;
            }
            return false;
        }
        ...
        private (ExpressionNode, bool) SortConjuncts(ExpressionNode node)
        {
            ...
            if (bin.Operator != "∧")
            {
                var (newLeft, nappliedLeft) = SortConjuncts(bin.Left);
                var (newRight, appliedRight) = SortConjuncts(bin.Right);
                return (new BinaryNode(bin.Operator, newLeft, newRight), nappliedLeft || appliedRight);
            }
        ...
        public void MakePDNFFromDNF()
        {
            ...
            if (!IsDNF(ast))
            {
                throw new Exception("Формула не является ДНФ=(");
            ...
            foreach (var disjunct in disjuncts)
            {
               ...
               .ToList()
                ...
                HashSet<string> additionalVarSet = new HashSet<string>(_variablesInOrder);
                additionalVarSet.ExceptWith(varsInDisjunct);
                List<string> additionalVars = additionalVarSet.OrderBy(x => x).ToList();
                ...
                _rpn = AstToRpn(CombineDisjuncts(multipleDisjuncts));
        ...
        private bool IsConjunctionOfVars(ExpressionNode node)
        {
            ...
            foreach (var conjunct in conjuncts)
            {
                ...
                if (conjunct is UnaryNode una2)
                {
                    if(!(una2.Operand is VariableNode))
                    {
                        return false;
                    }
                }
        ...
        public void ChangeNegativesToXorsInPDNF()
        {
            ...
            if (!IsDNF(ast))
            {
                throw new Exception("Формула не явдяется ДНФ=(");
            ...
            _rpn = AstToRpn(CombineWith(newDisjuncts, "⊕"));
        }
           ...
           foreach(var term in terms)
            {
                ...
                if(simple.Count > 0)
                {
                   ...
                   var newOneomplex =  CombineWith(newTermsInside, "⊕");
                    var newExpression = new List<ExpressionNode>();
                    newExpression.Add(newOneomplex);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var firstComlexTerms = ExtractExpressions(firstComplex, "⊕");
                        var secondComlexTerms = ExtractExpressions(secondComplex, "⊕");
                        var newComplex = new List<ExpressionNode>();
                        foreach(var term1 in firstComlexTerms)
                        {
                            foreach(var term2 in secondComlexTerms)
                            {
        ...
        private (ExpressionNode, bool) SimplifyConjuncts(ExpressionNode node)
        {
            if (node is BinaryNode bin)
            {
                if (bin.Operator == "∧")
                {
                   ...
                   if(!conjuncts.Any(x => x is ConstantNode con && con.Value))
                    {
